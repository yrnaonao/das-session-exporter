...
class PolarDBHandler:
    ...
    async def get_session_data_for_instance(self, instance: InstanceList) -> List[Dict[str, Any]]:
        ...
        logger.info(f"处理PolarDB实例: {instance.ins_id}")
        ...
        for node in nodes:
            ...
            # 第一次调用：获取会话信息，传入ins_id/node_id参数
            session_data = await self._get_mysql_session_async(client, instance.ins_id, node.node_id)
            if not session_data:
    ...
    async def _get_mysql_session_async(self, client, ins_id: str, node_id: str) -> Optional[Dict[str, Any]]:
        """
        异步获取MySQL会话信息 - 第一次调用
        传入ins_id/node_id参数
        """
